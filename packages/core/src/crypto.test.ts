...
describe("encryptSecret / decryptSecret roundtrip", () => {
  it("decrypts back to the original plaintext", () => {
    ...
    expect(decryptSecret(packed, key, aad)).toBe("AIzaSyExample-Plain-Key-Value");
  });
  ...
  it("rejects a ciphertext whose auth tag has been tampered with", () => {
    ...
    tag[0] = tag[0]! ^ 0xff;
    parts[2] = tag.toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
  ...
  it("rejects decryption with a different AAD (cross-row transplant)", () => {
    ...
    expect(() => decryptSecret(packed, key, apiKeyAad("user-B", "youtube"))).toThrow();
  });

  it("rejects decryption with a different provider in AAD", () => {
    const key = freshKey();
    const packed = encryptSecret("p", key, apiKeyAad("user-A", "youtube"));
    expect(() => decryptSecret(packed, key, apiKeyAad("user-A", "brave"))).toThrow();
  });
  ...
  it("rejects unknown version prefix", () => {
    ...
    expect(() => decryptSecret(swapped, key, apiKeyAad("u", "youtube"))).toThrow(
  ...
  it("rejects malformed payload (wrong segment count)", () => {
    const key = freshKey();
    expect(() => decryptSecret("v1.only-three", key, "aad")).toThrow(/malformed ciphertext/);
  });
});
