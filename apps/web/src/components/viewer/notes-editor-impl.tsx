...
function MainNote({
  ...
  useEffect(() => {
    if (editor && existing && editor.getHTML() !== existing.contentMd) {
      editor.commands.setContent(existing.contentMd, false);
    }
  }, [editor, existing?.contentMd, existing]);
  ...
  function insertTimecode() {
    ...
    editor.chain().focus().insertContent(`[${formatTime(seconds)}] `).run();
  }
  ...
  return (
    <li className="rounded-md border border-border bg-card p-3">
      <div className="mb-1 flex items-center justify-between text-xs">
        <span className="font-mono text-muted">{formatTime(note.timestampSeconds ?? 0)}</span>
