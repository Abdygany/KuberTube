...
describe("htmlToPlainText", () => {
  it("strips tags and decodes entities", () => {
    expect(htmlToPlainText("<p>Hello &amp; goodbye</p>")).toBe("Hello & goodbye");
  });
  it("drops scripts and styles entirely", () => {
    expect(htmlToPlainText("<script>alert(1)</script>visible<style>x{}</style>")).toBe("visible");
  });
...
describe("htmlToMarkdown", () => {
  ...
  it("converts <a href> to [text](url)", () => {
    expect(htmlToMarkdown('Visit <a href="https://example.com">site</a>')).toContain(
...
describe("truncateAtBoundary", () => {
  it("returns the input unchanged when under limit", () => {
    expect(truncateAtBoundary("hello", 100)).toEqual({ text: "hello", truncated: false });
  });
  ...
  it("backs up to a sentence boundary", () => {
    const long = "First sentence. Second sentence. Third sentence. Fourth sentence stops here.";
